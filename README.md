# wan2gp-blender-upsampler

This plugin is a tutorial implementation for WanGP temporal upsampler plugins.
It adds a simple frame interpolation method named **Blend**. The method supports
...
- Inherit `SimpleScaleSuffixMixin` when your serialized values follow the common
  `<method><scale>` convention, for example `blend2`.
- Implement `query_temporal_upsampler_def()` to declare display labels, method
  ids, supported multipliers, dropdown ordering, and the config key.
...
output, previous_last_frame, fps = handler.temporal_upsample("blend2", sample, None, 12)
assert output[:, :, 0, 0].tolist() == [[0, 5, 10, 15, 20]]
assert previous_last_frame[:, :, 0, 0].tolist() == [[20]]
assert fps == 24
```

After enabling the plugin in WanGP and restarting, **Blend** appears as a
